...
export async function apiSignIn(data: SignInCredential) {
    ...
    const response = await ApiService2.fetchData<SignInResponse>({
        url: '/auth/login',
        method: 'post',
        data: formData,
        headers: {
    ...
    if (response.data) {
        ...
        try {
            // Get user info to check role with explicit authorization header
            const userResponse = await ApiService2.fetchData<{role: string, id: string, username: string, email: string}>({
                url: '/user/me',
                method: 'get',
                headers: {
            ...
            // Return user data along with tokens for use in useAuth hook
            return {
                ...response,
            ...
            clearTokens();
            // If it's an authentication error, the user might not be admin
